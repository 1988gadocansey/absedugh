    ...
    public function pad_receiptno($receiptno){
       ...
       return str_pad($receiptno, 12, "0", STR_PAD_LEFT);
        ...
        if ($request->has('year') && trim($request->input('year')) != "") {
          ...
          // $yearSub=  explode('/',$request->input('year'));
          //       $yearSub=$yearSub[0];

          // $fee->where("YEAR", "like", '%'.$yearSub.'%');
        ...
        if ($request->has('from_date') && $request->has('to_date')) {
            ...
            $fee->whereBetween(\DB::raw('BANK_DATE'), array($request->input('from_date'), $request->input('to_date')));
        ...
        $data = $fee->orderBy('BANK_DATE', 'DESC')->paginate(1000);
