...
public function downloadError(Request $request, SystemController $sys )

    {
       ...
       // dd($resultb);
        ...
        $yearGP = $request->input("year");
        $combined = $request->input("combine");
        if ($combined == 'yes') {
            $combine = 'COMBINEDCODE';
        } else {
            $combine = 'PROGRAMMECODE';
        }
        ...
        $fac3 = $fac2->FACULTY;
         ...
         $nabptexYear = $sys->getNABPTEXyearGroup($year);
        //     $data = Models\StudentModel::where(function ($wquery) use($program, $year) { 
        //     $wquery->where("PROGRAMMECODE",$program)
        //     ->where("GRADUATING_GROUP",$year)->where("STATUS","!=","Admitted");})
        //     ->orWhere(function($wquery) use($program, $nabptexYear) {
        //     $wquery->where("PROGRAMMECODE",$program)->where(\DB::raw('SUBSTRING(`INDEXNO`, 3, 2)'), $nabptexYear)
        //     ->where("STATUS","!=","Admitted");})

        $dataWorking = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->where(function ($wquery) use($program, $year) { 
            $wquery->where("tpoly_students.PROGRAMMECODE", $program)
            ->where('tpoly_students.GRADUATING_GROUP',$year)
            ->where('tpoly_academic_record.grade','!=','E');})
            ->orWhere(function($wquery) use($program, $nabptexYear) {
            $wquery->where("tpoly_students.PROGRAMMECODE", $program)
            ->where(\DB::raw('SUBSTRING(tpoly_students.INDEXNO, 3, 2)'), $nabptexYear)
            ->where('tpoly_academic_record.grade','!=','E');})  
            ...
            $data = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine, $program)
            ->where('tpoly_status.year_group',$year)
            ->where('tpoly_academic_record.grade','!=','E') 
            ->orderBy("tpoly_status.action")   
            ->orderBy("tpoly_academic_record.indexno")
            ->orderBy("tpoly_academic_record.level")
            ->orderBy("tpoly_academic_record.sem")
            ->orderBy("tpoly_academic_record.resit")
            ->select('tpoly_academic_record.id', 'tpoly_academic_record.course', 'tpoly_academic_record.code', 'tpoly_academic_record.credits', 'tpoly_academic_record.student', 'tpoly_academic_record.indexno', 'tpoly_academic_record.total as mm', 'tpoly_academic_record.grade', 'tpoly_academic_record.gpoint', 'tpoly_academic_record.year', 'tpoly_academic_record.sem', \DB::raw('substr(tpoly_academic_record.level, 1, 3) as level'), 'tpoly_academic_record.yrgp', 'tpoly_academic_record.groups', 'tpoly_academic_record.lecturer', 'tpoly_academic_record.resit', 'tpoly_academic_record.dateRegistered', 'tpoly_academic_record.createdAt', 'tpoly_academic_record.updates', 'tpoly_academic_record.programme', \DB::raw('concat(tpoly_academic_record.code, tpoly_academic_record.indexno)'), 'tpoly_academic_record.total as tt', \DB::raw('concat(tpoly_academic_record.code, tpoly_academic_record.indexno, tpoly_academic_record.resit)'), 'tpoly_academic_record.total as yes', \DB::raw('concat(tpoly_students.INDEXNO, tpoly_status.action)'), 'tpoly_status.action')
            ->get();


                      

            $dataResit = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine, $program)
            ->where('tpoly_status.year_group',$year)
        ...
        return Excel::create($year.'_'.$programme, function ($excel) use ($data,$dataResit,$program,$kojoSen2,$yearcc,$yearGP,$year,$programme,$kojoSense,$dpt3,$fac3,$lectname, $sys, $combine){
   ...
   ;
            ...
            $excel->sheet('TP', function ($sheet) use ($data,$kojoSense,$program,$kojoSen2,$year,$programme,$dpt3,$fac3,$lectname, $combine) {
...
//});
            ...
            $excel->sheet('TP2', function ($sheet) use ($dataResit,$kojoSense,$program,$kojoSen2,$year,$programme,$dpt3,$fac3,$lectname, $combine) {
...
//});
        ...
        public function nabptexHead($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub)

        {

            $range = "A".$newHeaderRow.":AC".$newHeaderRow;


                                    $sheet->prependRow($newHeaderRow, array('NO','INDEXNO', 'NAME', 'CR', 'GP', 'GPA', 'CR', 'GP', 'GPA', 'CGPA', 'CR', 'GP', 'GPA', 'CGPA', 'CR', 'GP', 'GPA', 'CGPA', 'CR', 'GP', 'GPA', 'CGPA', 'CR', 'GP', 'GPA', 'CR', 'GP', 'CGPA', ''));


                                    $sheet->cells('D'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });

                                    $sheet->cells('E'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('F'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('G'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('H'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('I'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('J'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('K'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('L'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('M'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('N'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('O'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('P'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('Q'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });

                                    $sheet->cells('R'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('S'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('T'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('U'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('V'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('W'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('X'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('Y'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','medium','','');
                                   
                                    });
                                    $sheet->cells('Z'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    $sheet->cells('AA'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('','thin','','');
                                   
                                    });
                                    
                                    $sheet->cells('A'.$newHeaderRow.':AC'.$newHeaderRow, function($celB) {

                                    $celB->setBorder('thin','medium','medium','thin');
                                   
                                    });

                                    $sheet->prependRow($newHeaderRow, array('NO','INDEX NO', 'NAME', 'SEMESTER 1', '', '', 'SEMESTER 2', '', '', '', 'SEMESTER 3', '', '', '', 'SEMESTER 4', '', '', '', 'SEMESTER 5', '', '', '', 'SEMESTER 6', '', '', 'CUMMULATIVE', '', '', 'REMARKS'));
                                    $headnex = $newHeaderRow;
                                    $headnext = $headnex + 1;

                                    $sheet->mergeCells('D'.$newHeaderRow.':F'.$newHeaderRow);
                                    $sheet->mergeCells('G'.$newHeaderRow.':J'.$newHeaderRow);
                                    $sheet->mergeCells('K'.$newHeaderRow.':N'.$newHeaderRow);
                                    $sheet->mergeCells('O'.$newHeaderRow.':R'.$newHeaderRow);
                                    $sheet->mergeCells('S'.$newHeaderRow.':V'.$newHeaderRow);
                                    $sheet->mergeCells('W'.$newHeaderRow.':Y'.$newHeaderRow);
                                    $sheet->mergeCells('Z'.$newHeaderRow.':AB'.$newHeaderRow);
                                    $sheet->mergeCells('AC'.$newHeaderRow.':AC'.$headnext);
                                    $sheet->mergeCells('B'.$newHeaderRow.':B'.$headnext);
                                    $sheet->mergeCells('C'.$newHeaderRow.':C'.$headnext);
                                    $sheet->mergeCells('A'.$newHeaderRow.':A'.$headnext);

                                    $sheet->cells('D'.$newHeaderRow.':AB'.$kojoSense.'', function($celcenter) {
                                    ...
                                    $sheet->cells('AC'.$newHeaderRow, function($celcenter) {
                                    ...
                                    $sheet->cell('D'.$newHeaderRow, function($celB) {

                                
                                    $celB->setBorder('medium','medium','medium','medium');
                                   
                                    });
                                    $sheet->cell('G'.$newHeaderRow, function($celB) {

                               
                                    $celB->setBorder('medium','medium','medium','medium');
                                   
                                    });
                                    $sheet->cell('K'.$newHeaderRow, function($celB) {

                                
                                    $celB->setBorder('medium','medium','medium','medium');
                                   
                                    });
                                    $sheet->cell('O'.$newHeaderRow, function($celB) {

                                
                                 $celB->setBorder('medium','medium','medium','medium');
                                   
                            });
                            $sheet->cell('S'.$newHeaderRow, function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('medium','medium','medium','medium');
                                   
                            });
                            $sheet->cell('W'.$newHeaderRow, function($celB) {
                            ...
                            $sheet->cell('Z'.$newHeaderRow, function($celB) {
                            ...
                            $sheet->cell('AC'.$newHeaderRow, function($celB) {
                            ...
                            $headnex = $newHeaderRow;
                                ...
                                $sheet->cell('A'.$newHeaderRow.':AC'.$headnext, function($cell) {

                            //$cell->setAlignment('center');
                            $cell->setFontWeight('bold');
                            $cell->setFontSize(8);
                            });
                                $sheet->prependRow($newHeaderRow, array($remarks,'','','','PROGRAMME: '.$programme,'',));
                            $sheet->cell('A'.$newHeaderRow.':AC'.$newHeaderRow, function($cell) {
                            ...
                            $sheet->mergeCells('E'.$newHeaderRow.':Y'.$newHeaderRow);
                                $sheet->cell('E'.$newHeaderRow, function($cell) {

                            $cell->setAlignment('center');
                            
                            });
                                $sheet->setBorder($range, 'none');
                                $sheet->prependRow($newHeaderRow, array('','','','','DEPARTMENT: '.$dpt3,'','','','','','','','','','','','','','','','','','','','','','','','YEAR: '.$yearSub));
                            ...
                            $cell->setAlignment('center');
                                ...
                                $sheet->cell('AC'.$newHeaderRow, function($cell) {
                            ...
                            $cell->setAlignment('right');
                                ...
                                $sheet->mergeCells('B'.$newHeaderRow.':AC'.$newHeaderRow);
                                $sheet->cell('B'.$newHeaderRow, function($cell) {
                            ...
                            $cell->setFontSize(12);
                                ...
                                $sheet->setBorder($range, 'none');
                                $sheet->prependRow($newHeaderRow, array('','','','','','','','','','','','','','','','','','','','','','','','','','','','','Page'." ".' of'." ".'' ));
        ...
        public function nabptexFoot($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub)

        {
            ...
            $range = "A".$newHeaderRow.":AC".$newHeaderRow;
                            ...
                            $sheet->setBorder($range, 'none');
                            $sheet->prependRow($newHeaderRow, array(' External Examiner______________________________','','','','','','','','','','','Signature_______________________','','','','','','','','','','','Date_______________________'
                            ...
                            $sheet->prependRow($newHeaderRow, array(' Pro Vice Chancellor_____________________________','','','','','','','','','','','Signature_______________________','','','','','','','','','','','Date_______________________'
        ...
        public function nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub)

        {

            $range = "A".$newHeaderRow.":AE".$newHeaderRow;


                                    $sheet->prependRow($newHeaderRow, array('NO','INDEXNO', 'NAME', 'CR', 'NC', 'RS', 'RSA', 'CR', 'NC', 'RS', 'RSA', 'CR', 'NC', 'RS', 'RSA', 'CR', 'NC', 'RS', 'RSA', 'CR', 'NC', 'RS', 'RSA', 'CR', 'NC', 'RS', 'RSA', 'CR', 'RSA', 'CPA', ''));
                                    ...
                                    $sheet->cells('A'.$newHeaderRow.':AE'.$newHeaderRow, function($celB) {
                                    ...
                                    $sheet->mergeCells('D'.$newHeaderRow.':G'.$newHeaderRow);
                                    $sheet->mergeCells('H'.$newHeaderRow.':K'.$newHeaderRow);
                                    $sheet->mergeCells('L'.$newHeaderRow.':O'.$newHeaderRow);
                                    $sheet->mergeCells('P'.$newHeaderRow.':S'.$newHeaderRow);
                                    $sheet->mergeCells('T'.$newHeaderRow.':W'.$newHeaderRow);
                                    $sheet->mergeCells('X'.$newHeaderRow.':AA'.$newHeaderRow);
                                    $sheet->mergeCells('AB'.$newHeaderRow.':AD'.$newHeaderRow);
                                    $sheet->mergeCells('AE'.$newHeaderRow.':AE'.$headnext);
                                    $sheet->mergeCells('B'.$newHeaderRow.':B'.$headnext);
                                    $sheet->mergeCells('C'.$newHeaderRow.':C'.$headnext);
                                    $sheet->mergeCells('A'.$newHeaderRow.':A'.$headnext);

                                    $sheet->cells('D'.$newHeaderRow.':AD'.$kojoSense.'', function($celcenter) {
                                    ...
                                    $sheet->cells('AE'.$newHeaderRow, function($celcenter) {
                                    ...
                                    $sheet->cell('H'.$newHeaderRow, function($celB) {

                               
                                    $celB->setBorder('medium','medium','medium','medium');
                                   
                                    });
                                    $sheet->cell('L'.$newHeaderRow, function($celB) {

                                
                                    $celB->setBorder('medium','medium','medium','medium');
                                   
                                    });
                                    $sheet->cell('P'.$newHeaderRow, function($celB) {

                                
                                 $celB->setBorder('medium','medium','medium','medium');
                                   
                            });
                            $sheet->cell('T'.$newHeaderRow, function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('medium','medium','medium','medium');
                                   
                            });
                            $sheet->cell('X'.$newHeaderRow, function($celB) {
                            ...
                            $sheet->cell('AE'.$newHeaderRow, function($celB) {
                            ...
                            $headnex = $newHeaderRow;
                                ...
                                $sheet->cell('A'.$newHeaderRow.':AE'.$headnext, function($cell) {
                            ...
                            $cell->setFontSize(8);
                                ...
                                $sheet->prependRow($newHeaderRow, array($remarks,'','','','PROGRAMME: '.$programme,'',));
                            $sheet->cell('A'.$newHeaderRow.':AE'.$newHeaderRow, function($cell) {
                            ...
                            $sheet->mergeCells('E'.$newHeaderRow.':AA'.$newHeaderRow);
                                $sheet->cell('E'.$newHeaderRow, function($cell) {
                            ...
                            $cell->setAlignment('center');
                                ...
                                $sheet->cell('AE'.$newHeaderRow, function($cell) {
                            ...
                            $cell->setAlignment('right');
                                ...
                                $sheet->setBorder($range, 'none');
                                $sheet->prependRow($newHeaderRow, array('','CUMULATIVE GRADE POINTS FOR HIGHER NATIONAL DIPLOMA CBT PROGRAMMES IN POLYTECHNICS / TECHNICAL UNIVERSITIES'));
                                $sheet->mergeCells('B'.$newHeaderRow.':AE'.$newHeaderRow);
                                $sheet->cell('B'.$newHeaderRow, function($cell) {
        ...
        public function nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub)
    ...
    {
if ($sys->getUserLevel((@\Auth::user()->department),"download_nabptex") == '1' || $sys->getUserLevel((@\Auth::user()->role),"download_nabptex") == '1') {
...
//});
            ...
            $nabptexYear = $sys->getNABPTEXyearGroup($year);
            $data = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            //->where("tpoly_students.INDEXNO","07159016")
            ->where("tpoly_status.year_group",$year)
            ->where("tpoly_status.action","!=","Admitted")
            ->orderBy("tpoly_students.TRAIL")
            ->orderBy("tpoly_students.SUP")
            ->orderBy(\DB::raw('FIELD(tpoly_status.action, "Alumni", "In school", "Abandoned", "Withdrawn", "Rusticated", "Deferred")'))
            
            //ORDER BY FIELD(priority, "core", "board", "other")
            ->orderBy("tpoly_students.INDEXNO")
            ->select('tpoly_students.INDEXNO', \DB::raw('concat(tpoly_students.SURNAME,", ",tpoly_students.FIRSTNAME," ",tpoly_students.OTHERNAMES) NAME'))
            ->get();


            $dataNotMixed = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            //->where("tpoly_students.INDEXNO","07159016")
            ->where("tpoly_status.year_group",$year)
            ->where("tpoly_status.action","!=","Admitted")
            ...
            $kojoSense = count($data)+1;
                ...
                $excel->sheet('RSA', function ($sheet) use ($data,$dataNotMixed,$kojoSense,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $combine) 
                    {
            ...
            $excel->sheet('NAB_RSA', function ($sheet) use ($data,$kojoSense,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $combine) 
                    {
                    ...
                    $sheet->setWidth(array(
                        ...
                        $sheet->fromArray($data, Null, 'B1', true, false);
                for($k=1;$k<$kojoSense;$k++){
                    ...
                    $sheet->setCellValue('A'.$kIdCount.'',$k);
                        ...
                        $kojoCellBeauty = $kojoSense - 1;
                        $sheet->cells('A1:AE'.$kojoSense.'', function($cells) 
                            {
                        ...
                        for($k=1;$k<$kojoSense;$k++)
                            {
                            $sheet->setCellValue('D'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('E'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('F'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('G'.$k.'','=IFERROR(F'.$k.'/E'.$k.',"")');
                            $sheet->getStyle('G'.$k.'')->getNumberFormat()->setFormatCode('0.00'); 


                            $sheet->setCellValue('H'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('I'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('J'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',100)');

                            $sheet->setCellValue('K'.$k.'','=IFERROR(J'.$k.'/I'.$k.',"")');
                            $sheet->getStyle('K'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('L'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('M'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('N'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('O'.$k.'','=IFERROR(N'.$k.'/M'.$k.',"")');
                            $sheet->getStyle('O'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('P'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('Q'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('R'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"200","600"})');

                            $sheet->setCellValue('S'.$k.'','=IFERROR(R'.$k.'/Q'.$k.',"")');
                            $sheet->getStyle('S'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('T'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('U'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('V'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('W'.$k.'','=IFERROR(V'.$k.'/U'.$k.',"")');
                            $sheet->getStyle('W'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('X'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('Y'.$k.'','=COUNTIFS(TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('Z'.$k.'','=SUMIFS(TP!G8:TP!G'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',"300")');
                            ...
                            $sheet->setCellValue('AC'.$k.'','=SUM(SUMIF(G'.$k.',">0",G'.$k.'),SUMIF(K'.$k.',">0",K'.$k.'),SUMIF(O'.$k.',">0",O'.$k.'),SUMIF(S'.$k.',">0",S'.$k.'),SUMIF(W'.$k.',">0",W'.$k.'),SUMIF(AA'.$k.',">0",AA'.$k.'))');
                            $sheet->getStyle('AC'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('AD'.$k.'','=IFERROR(AC'.$k.'/AB'.$k.',"")');
                            $sheet->getStyle('AD'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            //=IF(AC3>6,"",IF(AC3>=4,"CD",IF(AC3>=3,"CM",IF(AC3>=2,"C",IF(AC3<2,"NC","")))))

                            //$sheet->setCellValue('AD'.$k.'','=IF(AC'.$k.'>6,"",IF(AC'.$k.'>3.994,"CD",IF(AC'.$k.'>2.994,"CM",IF(AC'.$k.'>1.994,"C",IF(AC'.$k.'>=0,"NC","")))))');

                            $sheet->setCellValue('AE'.$k.'','=IFERROR(VLOOKUP($B'.$k.'&"Abandoned",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Deferred",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Rusticated",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Withdrawn",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"no",TP2!$B$8:TP2!$C$'.$kojoSen2.',2,FALSE),IF(AD'.$k.'>6,"",IF(AD'.$k.'>3.994,"COMPETENT WITH DISTINCTION",IF(AD'.$k.'>2.994,"COMPETENT WITH MERIT",IF(AD'.$k.'>1.994,"COMPETENT",IF(AD'.$k.'>=0,"NOT YET COMPETENT",""))))))))))');

                           $sheet->getStyle('AE'.$k.'')->getAlignment()->setWrapText(true);


                            $sheet->getStyle('AE'.$k.'')->getAlignment()->setWrapText(true);
                            $sheet->cells('AE'.$k.'', function($celcenter) {

                                // manipulate the cell
                                $celcenter->setAlignment('left');
                            ...
                            $sheet->cells('A1:A'.$kojoCellBeauty.'', function($celcenter) {
                            ...
                            $sheet->cells('D1:AD'.$kojoCellBeauty.'', function($celcenter) {
                            ...
                            $sheet->cells('AE1:AE'.$kojoCellBeauty.'', function($celcenter) {
                            ...
                            $sheet->setBorder('A1:AE'.$kojoCellBeauty.'', 'thin'); 

                            $sheet->cells('K1:K'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('C1:C'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('G1:G'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('O1:O'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('S1:S'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('W1:W'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('AA1:AA'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            }); 
                            $sheet->cells('AE1:AE'.$kojoCellBeauty.'', function($celB) {
                            ...
                            $sheet->cells('B1:B'.$kojoCellBeauty.'', function($celB) {
                            ...
                            $sheet->cells('AD1:AD'.$kojoCellBeauty.'', function($celB) {
            ...
            $dataAllStudents = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year) 
            ->where('tpoly_status.action',"!=",'Admitted')
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataAllStudents = count($dataAllStudents);

               

            $dataDeferred = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Deferred') 
            ->where('tpoly_status.action',"!=",'Admitted')
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataDeferred = count($dataDeferred);

            $dataDeferred2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action','Deferred') 
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataDeferred2 = count($dataDeferred2);

            $dataRusticate = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Rusticated')
            ->where('tpoly_status.action',"!=",'Deferred') 
            ->where('tpoly_status.action',"!=",'Admitted')           
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataRusticate = count($dataRusticate);

            $dataRusticate2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action','Rusticated')            
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataRusticate2 = count($dataRusticate2);

            $dataWithdrawn = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Rusticated')
            ->where('tpoly_status.action',"!=",'Deferred')
            ->where('tpoly_status.action',"!=",'Withdrawn') 
            ->where('tpoly_status.action',"!=",'Admitted')           
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataWithdrawn = count($dataWithdrawn); 

            $dataWithdrawn2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)
            ->where('tpoly_status.action','Withdrawn')            
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataWithdrawn2 = count($dataWithdrawn2);

            $dataAbandoned = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Rusticated')
            ->where('tpoly_status.action',"!=",'Deferred')
            ->where('tpoly_status.action',"!=",'Withdrawn')
            ->where('tpoly_status.action',"!=",'Abandoned') 
            ->where('tpoly_status.action',"!=",'Admitted')           
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataAbandoned = count($dataAbandoned);

            $dataAbandoned2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)
            ->where('tpoly_status.action','Abandoned')            
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataAbandoned2 = count($dataAbandoned2);

            $dataTrail = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Rusticated')
            ->where('tpoly_status.action',"!=",'Deferred')
            ...
            ->where('tpoly_status.action',"!=",'Abandoned')
            ->where('tpoly_status.action',"!=",'Admitted')
            ->where('tpoly_students.TRAIL',"!=",'yes')
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataTrail = count($dataTrail); 

            $dataTrail2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year) 
            ->where('tpoly_students.TRAIL','yes')
            ->where('tpoly_status.action',"!=",'Rusticated')
            ...
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataTrail2 = count($dataTrail2); 


            $dataSupplementary = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year)            
            ->where('tpoly_status.action',"!=",'Rusticated')
            ->where('tpoly_status.action',"!=",'Deferred')
            ...
            ->where('tpoly_status.action',"!=",'Admitted')
            ->where('tpoly_students.TRAIL',"!=",'yes')
            ->where('tpoly_students.SUP',"!=",'1')
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataSupplementary = count($dataSupplementary); 

            $dataSupplementary2 = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year) 
            ->where('tpoly_students.SUP','1')
            ->where('tpoly_students.TRAIL','no')
            ->select('tpoly_students.INDEXNO')            
            ->get();
            $dataSupplementary2 = count($dataSupplementary2);


            $dataAll = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine,$program)
            ->where("tpoly_status.year_group",$year) 
            //->where('TRAIL','yes')
        ...
        $yearSub1 = explode('/',$year);
        $yearSub = $yearSub1[1];
                           

                                                    $remarks = "GRADUATING"; 
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);

                            if ($dataAllStudents > 0) 
                                        {
                                                    $remarks = "";                                    
                                                    $newHeaderRow = $dataAllStudents + 8;
                                                    $kojoSense+=14;
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                       
                                                
                                        }

                            if ($dataDeferred > 0 && $dataDeferred2 > 0) 
                                        {
                                                    $remarks = "DEFERRED";                                    
                                                    $newHeaderRow = $dataDeferred + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataDeferred2;$k++){
                                                
                                                
                                                $sheet->setCellValue('A'.$kIdCount.'',$k);
                                                $sheet->setCellValue('AD'.$kIdCount.'','');
                                                $sheet->setCellValue('AC'.$kIdCount.'','');
                                                $sheet->setCellValue('AB'.$kIdCount.'','');
                                                $kIdCount = $kIdCount+ 1;
                                                }
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                        ...
                                        $shortHead = 0;
                                        $setCount = 1;
                            while ($dataDeferred2 > 25) {
                                            $dataDeferred2 = $dataDeferred2 - 25;
                                            $remarks = "DEFERRED";                                    
                                                    $newHeaderRow = (25*$setCount) + $dataDeferred + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                                                    $setCount = $setCount + 1;
                                        }    

                            if ($dataRusticate > 0 && $dataRusticate2 > 0) 
                                        {
                                                    $remarks = "RUSTICATED";                                    
                                                    $newHeaderRow = $dataRusticate + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataRusticate2;$k++){
                                         ...
                                         $shortHead = 0;
                                        $setCount = 1;
                            while ($dataRusticate2 > 25) {
                                            $dataRusticate2 = $dataRusticate2 - 25;
                                            $remarks = "RUSTICATED";                                    
                                                    $newHeaderRow = (25*$setCount) + $dataRusticate + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                            ...
                            if ($dataWithdrawn > 0 && $dataWithdrawn2 > 0) 
                                        {
                                                    $remarks = "WITHDRAWN";                                    
                                                    $newHeaderRow = $dataWithdrawn + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataWithdrawn2;$k++){
                                         ...
                                         $shortHead = 0;
                                        $setCount = 1;
                            while ($dataWithdrawn2 > 25) {
                                            $dataWithdrawn2 = $dataWithdrawn2 - 25;
                                            $remarks = "WITHDRAWN";                                    
                                                    $newHeaderRow = (25*$setCount) + $dataWithdrawn + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                            ...
                            if ($dataAbandoned > 0 && $dataAbandoned2 > 0) 
                                        {
                                                    $remarks = "ABANDONED";                                    
                                                    $newHeaderRow = $dataAbandoned + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataAbandoned2;$k++){
                                         ...
                                         $shortHead = 0;
                                        $setCount = 1;
                            while ($dataAbandoned2 > 25) {
                                            $dataAbandoned2 = $dataAbandoned2 - 25;
                                            $remarks = "ABANDONED";                                    
                                                    $newHeaderRow = (25*$setCount) + $dataAbandoned + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                            ...
                            if ($dataTrail > 0 && $dataTrail2 > 0) 
                                        {
                                            
                                                    $remarks = "TRAILING";                                    
                                                    $newHeaderRow = $dataTrail + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataTrail2;$k++){
                                        ...
                                        $setCount = 1;
                            while ($dataTrail2 > 15) {
                                            $dataTrail2 = $dataTrail2 - 15;
                                            $remarks = "TRAILING";                                    
                                                    $newHeaderRow = (15*$setCount) + $dataTrail + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                            ...
                            if ($dataSupplementary > 0 && $dataSupplementary2 > 0) 
                                        {
                                                    $remarks = "SUPPLEMENTARY";                                    
                                                    $newHeaderRow = $dataSupplementary + 8;
                                                    $kojoSense+=14;
                                                    $kID = $newHeaderRow;
                                                    $kIdCount = $kID;
                                                    for($k=1;$k<=$dataSupplementary2;$k++){
                                        ...
                                        $shortHead = 0;
                                        $setCount = 1;
                            while ($dataSupplementary2 > 25) {
                                            $dataSupplementary2 = $dataSupplementary2 - 25;
                                            $remarks = "SUPPLEMENTARY";                                    
                                                    $newHeaderRow = (25*$setCount) + $dataSupplementary + 8 + ($setCount * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                                        ...
                                        $shortHead = 0;
                                        $setCount = 1;
                            while ($dataSupplementary > 25) {
                                            $dataSupplementary = $dataSupplementary - 25;
                                            $remarks = "GRADUATING";                                    
                                                    $newHeaderRow = (25*$setCount) + 8 + ($shortHead * 15);
                                                    $kojoSense+=14;
                                                    $this->nabptexHeadRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $this->nabptexFootRSA($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                                                    $shortHead = $shortHead + 1;
                    ...
                    $excel->sheet('CGPA', function ($sheet) use ($data,$dataNotMixed,$kojoSense,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $combine) 
                    {
                    //CGPA format begins here
                    $sheet->setWidth(array(
                        'A'     =>  15,
            ...
            $excel->sheet('NABPTEX', function ($sheet) use ($data,$kojoSense,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $combine) 
                    {
                    //CGPA format begins here
                    $sheet->setWidth(array(
                        'A'     =>  3.5,
                ...
                for($k=1;$k<$kojoSense;$k++){
                    ...
                    $sheet->setCellValue('A'.$kIdCount.'',$k);
                        ...
                        for($k=1;$k<$kojoSense;$k++)
                            ...
                            //$sheet->setCellValue('E'.$k.'','0');
                            //$sheet->setCellValue('F'.$k.'','0');
                            $sheet->setCellValue('D'.$k.'','=SUM(SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"100","500"}))');

                            $sheet->setCellValue('E'.$k.'','=SUM(SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"100","500"}))');
                            $sheet->getStyle('E'.$k.'')->getNumberFormat()->setFormatCode('0.00');
                            ...
                            $sheet->setCellValue('G'.$k.'','=SUM(SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"100","500"}))');

                            $sheet->setCellValue('H'.$k.'','=SUM(SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"100","500"}))');
                            $sheet->getStyle('H'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('I'.$k.'','=IFERROR(H'.$k.'/G'.$k.',"")');
                            $sheet->getStyle('I'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('J'.$k.'','=IFERROR((H'.$k.'+E'.$k.')/(G'.$k.'+D'.$k.'),"")');
                            $sheet->getStyle('J'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('K'.$k.'','=SUM(SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"200","600"}))');

                            $sheet->setCellValue('L'.$k.'','=SUM(SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',{"200","600"}))');
                            $sheet->getStyle('L'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('M'.$k.'','=IFERROR(L'.$k.'/K'.$k.',"")');
                            $sheet->getStyle('M'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('N'.$k.'','=IFERROR((H'.$k.'+E'.$k.'+L'.$k.')/(G'.$k.'+D'.$k.'+K'.$k.'),"")');
                            $sheet->getStyle('N'.$k.'')->getNumberFormat()->setFormatCode('0.00');
                            
                            $sheet->setCellValue('O'.$k.'','=SUM(SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"200","600"}))');

                            $sheet->setCellValue('P'.$k.'','=SUM(SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',{"200","600"}))');
                            $sheet->getStyle('P'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('Q'.$k.'','=IFERROR(P'.$k.'/O'.$k.',"")');
                            $sheet->getStyle('Q'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('R'.$k.'','=IFERROR((H'.$k.'+E'.$k.'+L'.$k.'+P'.$k.')/(G'.$k.'+D'.$k.'+K'.$k.'+O'.$k.'),"")');
                            $sheet->getStyle('R'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('S'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('T'.$k.'','=SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',1,TP!L8:TP!L'.$kojoSen2.',"300")');
                            $sheet->getStyle('T'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('U'.$k.'','=IFERROR(T'.$k.'/S'.$k.',"")');
                            $sheet->getStyle('U'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('V'.$k.'','=IFERROR((H'.$k.'+E'.$k.'+L'.$k.'+P'.$k.'+T'.$k.')/(G'.$k.'+D'.$k.'+K'.$k.'+O'.$k.'+S'.$k.'),"")');
                            $sheet->getStyle('V'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('W'.$k.'','=SUMIFS(TP!D8:TP!D'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',"300")');

                            $sheet->setCellValue('X'.$k.'','=SUMIFS(TP!I8:TP!I'.$kojoSen2.',TP!F8:TP!F'.$kojoSen2.',B'.$k.',TP!K8:TP!K'.$kojoSen2.',2,TP!L8:TP!L'.$kojoSen2.',"300")');
                            $sheet->getStyle('X'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('Y'.$k.'','=IFERROR(X'.$k.'/W'.$k.',"")');
                            $sheet->getStyle('Y'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            //$sheet->setCellValue('Y'.$k.'','=(G'.$k.'+D'.$k.'+K'.$k.'+O'.$k.'+S'.$k.'+W'.$k.')/(F'.$k.'+C'.$k.'+J'.$k.'+N'.$k.'+R'.$k.'+V'.$k.')');

                            $sheet->setCellValue('Z'.$k.'','=G'.$k.'+D'.$k.'+K'.$k.'+O'.$k.'+S'.$k.'+W'.$k);

                            $sheet->setCellValue('AA'.$k.'','=H'.$k.'+E'.$k.'+L'.$k.'+P'.$k.'+T'.$k.'+X'.$k);
                            $sheet->getStyle('AA'.$k.'')->getNumberFormat()->setFormatCode('0.00');

                            $sheet->setCellValue('AB'.$k.'','=IFERROR(AA'.$k.'/Z'.$k.',"")');
                            $sheet->getStyle('AB'.$k.'')->getNumberFormat()->setFormatCode('0.00');
                            

                            $sheet->setCellValue('AC'.$k.'','=IFERROR(VLOOKUP($B'.$k.'&"Abandoned",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Deferred",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Rusticated",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"Withdrawn",TP!$Y$8:TP!$Z$'.$kojoSen2.',2,FALSE),IFERROR(VLOOKUP($B'.$k.'&"no",TP2!$B$8:TP2!$C$'.$kojoSen2.',2,FALSE),IF(AB'.$k.'>5,"",IF(AB'.$k.'>3.994,"FIRST CLASS",IF(AB'.$k.'>2.994,"SECOND CLASS UPPER DIVISION",IF(AB'.$k.'>1.994,"SECOND CLASS LOWER DIVISION",IF(AB'.$k.'>1.494,"PASS",IF(AB'.$k.'<=1.494,"FAIL","")))))))))))');

                            $sheet->getStyle('AC'.$k.'')->getAlignment()->setWrapText(true);
                            $sheet->cells('AC'.$k.'', function($celcenter) {
                            ...
                            $sheet->cells('D1:AB'.$kojoCellBeauty.'', function($celcenter) {
                            ...
                            $sheet->cells('AC1:AC'.$kojoCellBeauty.'', function($celcenter) {
                            ...
                            $sheet->setBorder('A1:AC'.$kojoCellBeauty.'', 'thin'); 

                            $sheet->cells('J1:J'.$kojoCellBeauty.'', function($celB) {
                            ...
                            $sheet->cells('F1:F'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('N1:N'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('R1:R'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('V1:V'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            });
                            $sheet->cells('Y1:Y'.$kojoCellBeauty.'', function($celB) {

                                // manipulate the cell
                                 $celB->setBorder('','medium','thin','');
                                   
                            }); 
                            $sheet->cells('AC1:AC'.$kojoCellBeauty.'', function($celB) {
                            ...
                            $sheet->cells('AB1:AB'.$kojoCellBeauty.'', function($celB) {
...
//dd($dataTrail, $dataTrail2);
        ...
        $yearSub = $yearSub1[1];
                                                    ...
                                                    $remarks = "GRADUATING"; 
                                                    $this->nabptexHead($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                            ...
                            if ($dataAllStudents > 0) 
                                        {
                                                    ...
                                                    $newHeaderRow = $dataAllStudents + 8;
                                                    $kojoSense+=14;
                                                    $this->nabptexFoot($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
                            ...
                            if ($dataDeferred > 0 && $dataDeferred2 > 0) 
                                        {
                                                ...
                                                $sheet->setCellValue('A'.$kIdCount.'',$k);
                                                $sheet->setCellValue('Z'.$kIdCount.'','');
                                                $sheet->setCellValue('AA'.$kIdCount.'','');
                                                $sheet->setCellValue('AB'.$kIdCount.'','');
                                                $kIdCount = $kIdCount+ 1;
                                                }
                                                    $this->nabptexHead($newHeaderRow, $kojoSense, $sheet, $dpt3, $programme, $remarks, $yearSub);
...
$courseMACS1 = Models\MountedCourseModel::join('tpoly_courses','tpoly_courses.COURSE_CODE', '=', 'tpoly_mounted_courses.COURSE_CODE')->where('tpoly_mounted_courses.YEAR_GROUP', $yearGP)
...
$datafuck = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine, $program)
            ->where('tpoly_status.year_group',$year)
            ->where('tpoly_academic_record.grade','!=','E')
            ->orderBy("tpoly_academic_record.level")
            ...
            $dataSelectBlank = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
            ->where("tpoly_students.".$combine, $program)
            ->where('tpoly_status.year_group',$year)
            ->where('tpoly_academic_record.grade','!=','E')
            ->orderBy("tpoly_academic_record.level")
            ...
            ->orderBy("tpoly_academic_record.code")
            //->groupBY("tpoly_academic_record.code")
            ->select(\DB::raw('concat(tpoly_academic_record.code, tpoly_academic_record.indexno) as blank'))
            ->get();

            $blankA = '';

            foreach($dataSelectBlank as $row){
            
            $blankA=($row['blank']).','.$blankA;

        }
        $explode_blankA = array_map('strval', explode(',',$blankA));


        $dataSelectNotinschool = Models\StudentModel::join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')->where("tpoly_students.".$combine, $program)
            ->where('tpoly_status.year_group',$year)
            ->where('tpoly_status.action','!=','In school')
            ->where('tpoly_status.action','!=','Alumni')
            ->where('tpoly_status.action','!=','Admitted')
            ->orderBy("tpoly_students.INDEXNO")
           
            //->groupBY("tpoly_academic_record.code")
            ->select('tpoly_students.INDEXNO AS out')
            ->get();

            $outA = '';

            foreach($dataSelectNotinschool as $row){
            
            $outA=($row['out']).','.$outA;

        }
        $explode_Notinschool = array_map('strval', explode(',',$outA));
       //dd($blankA);
            //dd($year);
...
if ($rsaProgram == 'RSA' ) {
        ...
        $explode_fuck = array_map('strval', explode(',',$fuck));
            ...
            #excel for headers
            @$excel->sheet('MACS', function ($sheet) use ($data,$dataNotMixed,$courseMACS1,$kojoSense,$fuck,$fuckcount,$fuckcountr1,$fuckcountr2,$fuckcountr3,$fuckcountr4,$fuckcountr5,$fuckcountr6,$fuckcountr7,$fuckcountr8,$fucksem1,$fucksem2,$fucksem3,$fucksem4,$fucksem5,$fucksem6,$fucksem7,$fucksem8,$fucksem11,$fucksem21,$fucksem31,$fucksem41,$fucksem51,$fucksem61,$fucksem71,$fucksem81,$explode_fuck,$explode_blankA,$explode_Notinschool,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $preyear, $combine) 
                {
                        ...
                        for($k=2;$k<$kojoSense+1;$k++)
                            ...
                            $codeSelect = $sheet->getCell(''.$alpha.'1')->getValue();
                            $indexSelect = $sheet->getCell(''.'A'.$k)->getValue();
                            $conSelect = $codeSelect.$indexSelect;
                            //dd($conSelect,$alpha,$indexSelect);                            
                            
                            if ($value == 'PA' || $value == 'CPA') {
                                $sheet->getStyle(''.$alpha.$k.'')->getNumberFormat()->setFormatCode('0.00');
                            }

                            if (!in_array($conSelect, $explode_blankA) && $value != 'PA' && $value != 'CPA') 
                           {
                            @$sheet->cell(''.$alpha.$k.'', function($cell) {
                                @$cell->setBackground('#CE1111');
                                });
                             }

                             if (in_array($indexSelect, $explode_Notinschool) && $value != 'PA' && $value != 'CPA') 
                           {
                            @$sheet->cell(''.$alpha.$k.'', function($cell) {
                                @$cell->setBackground('#4F87F8');
                                });
                             }
               ...
               //   # code...
                        ...
                        //   } 
                         ...
                         if ($e == 1500) {
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell('C'.$k_fuck1.'', function($cell) {
                          ...
                          if ($e == 2500 and !empty($fucksem11) and !is_numeric($fucksem11)) {
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem11.$k_fuck2.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g2 = $g2 + $g; 
                        ...
                        if ($e == 1600 and !empty($fucksem21) and !is_numeric($fucksem21)) {
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem21.$k_fuck3.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g3 = $g3 + $g;  
                         ...
                         if ($e == 2600 and !empty($fucksem31) and !is_numeric($fucksem31)) {
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem31.$k_fuck4.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g4 = $g4 + $g;  
                        ...
                        if ($e == 1200 and !empty($fucksem21) and !is_numeric($fucksem21)) {
                         ...
                         if ($e == 1300 and !empty($fucksem41) and !is_numeric($fucksem41)) {
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem41.$k_fuck5.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g5 = $g5 + $g;  
                         ...
                         if ($e == 2300 and !empty($fucksem51) and !is_numeric($fucksem51)) {  
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem51.$k_fuck6.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g6 = $g6 + $g; 
                        ...
                        if ($e == 1400 and !empty($fucksem61) and !is_numeric($fucksem61)) {   
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem61.$k_fuck7.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g7 = $g7 + $g; 
                        ...
                        if ($e == 2400 and !empty($fucksem71) and !is_numeric($fucksem71)) {  
                            ...
                            if (!in_array($b, $explode_fuck)) 
                           {
                            @$sheet->cell(''.$fucksem71.$k_fuck8.'', function($cell) {
                                @$cell->setFontColor('#990000');
                                });
                             }
                            $g8 = $g8 + $g; 
            ...
            @$excel->sheet('MACS', function ($sheet) use ($data,$dataNotMixed,$courseMACS1,$kojoSense,$fuck,$fuckcount,$fuckcountr1,$fuckcountr2,$fuckcountr3,$fuckcountr4,$fuckcountr5,$fuckcountr6,$fuckcountr7,$fuckcountr8,$fucksem1,$fucksem2,$fucksem3,$fucksem4,$fucksem5,$fucksem6,$fucksem7,$fucksem8,$fucksem11,$fucksem21,$fucksem31,$fucksem41,$fucksem51,$fucksem61,$fucksem71,$fucksem81,$explode_fuck,$explode_blankA,$explode_Notinschool,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $preyear, $combine) 
                {
                        ...
                        for($k=2;$k<$kojoSense+1;$k++)
                            ...
                            if (!in_array($conSelect, $explode_blankA) && $value != 'GPA' && $value != 'CGPA') 
                           {
                            @$sheet->cell(''.$alpha.$k.'', function($cell) {
                                @$cell->setBackground('#CE1111');
                                });
                             ...
                             if (in_array($indexSelect, $explode_Notinschool) && $value != 'GPA' && $value != 'CGPA') 
                           {
                            @$sheet->cell(''.$alpha.$k.'', function($cell) {
                                @$cell->setBackground('#4F87F8');
                                });
...
//dd($kojoSense);
            #excel for headers
            @$excel->sheet('RESIT', function ($sheet) use ($data,$dataNotMixed,$courseMACS1,$kojoSense,$fuck,$fuckcount,$fucksem1,$fucksem2,$fucksem3,$fucksem4,$fucksem5,$fucksem6,$fucksem7,$fucksem8,$fucksem11,$fucksem21,$fucksem31,$fucksem41,$fucksem51,$fucksem61,$fucksem71,$fucksem81,$explode_fuck,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $checkResit1,$checkResit2,$checkResit3,$checkResit4,$checkResit5,$checkResit6,$checkResit7,$checkResit8, $combine) 
                {
...
$courseMACS1 = Models\MountedCourseModel::join('tpoly_courses','tpoly_courses.COURSE_CODE', '=', 'tpoly_mounted_courses.COURSE_CODE')->join('users','tpoly_mounted_courses.LECTURER', '=', 'users.fund')->where('tpoly_mounted_courses.YEAR_GROUP', $yearGP)
                ->where('tpoly_mounted_courses.PROGRAMME', $program)
                ->orderBy('tpoly_mounted_courses.COURSE_LEVEL')
                ->orderBy('tpoly_mounted_courses.COURSE_SEMESTER')
                ->orderBy('tpoly_mounted_courses.COURSE_CODE')
...
$datafuck = Models\AcademicRecordsModel::join('tpoly_students', 'tpoly_academic_record.indexno', '=', 'tpoly_students.INDEXNO')->join('tpoly_status','tpoly_students.INDEXNO', '=', 'tpoly_status.student')
        ...
        $explode_fuck = array_map('strval', explode(',',$fuck));
            ...
            @$excel->sheet('STATS', function ($sheet) use ($data,$courseMACS1,$kojoSense,$fuck,$fuckcount,$fuckcountr1,$fuckcountr2,$fuckcountr3,$fuckcountr4,$fuckcountr5,$fuckcountr6,$fuckcountr7,$fuckcountr8,$fucksem1,$fucksem2,$fucksem3,$fucksem4,$fucksem5,$fucksem6,$fucksem7,$fucksem8,$fucksem11,$fucksem21,$fucksem31,$fucksem41,$fucksem51,$fucksem61,$fucksem71,$fucksem81,$explode_fuck,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $preyear, $combine) 
                {
                        ...
                        $sheet->setCellValue(''.$statsClass.'9','Abandoned : ');
                        $sheet->setCellValue(''.$statsClass.'10','Withdrawn : ');
                        $sheet->setCellValue(''.$statsClass.'11','Deferred : ');
                        $sheet->setCellValue(''.$statsClass.'12','Rusticated : ');
                        $sheet->setCellValue(''.$statsClass.'13','Resit : ');
                        $sheet->setCellValue(''.$statsClass.'14','Unknown : ');

                        $statsClassAlign = $statsClass;

                        $Allstudent = $kwhykojoSense;
                        $TotallAllstudent = $Allstudent - 7;

                        $statsClassCount = ++$statsClass;
                        $sheet->setCellValue(''.$statsClassCount.'3','No');
                        $sheet->setCellValue(''.$statsClassCount.'4','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"First Class")');
                        $sheet->setCellValue(''.$statsClassCount.'5','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Second Class Upper Division")');
                        $sheet->setCellValue(''.$statsClassCount.'6','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Second Class Lower Division")');
                        $sheet->setCellValue(''.$statsClassCount.'7','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Pass")');
                        $sheet->setCellValue(''.$statsClassCount.'8','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Fail")');
                        $sheet->setCellValue(''.$statsClassCount.'9','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Abandoned")');
                        $sheet->setCellValue(''.$statsClassCount.'10','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Withdrawn")');
                        $sheet->setCellValue(''.$statsClassCount.'11','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Deferred")');
                        $sheet->setCellValue(''.$statsClassCount.'12','=COUNTIF(CGPA!AB8:CGPA!AB'.$kwhykojoSense.',"Rusticated")');
                        $sheet->setCellValue(''.$statsClassCount.'13','='.$TotallAllstudent.'-(SUM('.$statsClassCount.'4:'.$statsClassCount.'12))');
                        $sheet->setCellValue(''.$statsClassCount.'14',0);
            ...
            //});               
                            //$statsClassAlign = $statsClass--;
                            $sheet->cells(''.$statsClassAlign.'10:'.$statsClassAlign.'20', function($cells) {

                            $cells->setAlignment('right');
                            });

                            $sheet->cells(''.$statsClassCount.'9:'.$statsClassCount.'20', function($cells) {
            ...
            @$excel->sheet('STATS', function ($sheet) use ($data,$courseMACS1,$kojoSense,$fuck,$fuckcount,$fuckcountr1,$fuckcountr2,$fuckcountr3,$fuckcountr4,$fuckcountr5,$fuckcountr6,$fuckcountr7,$fuckcountr8,$fucksem1,$fucksem2,$fucksem3,$fucksem4,$fucksem5,$fucksem6,$fucksem7,$fucksem8,$fucksem11,$fucksem21,$fucksem31,$fucksem41,$fucksem51,$fucksem61,$fucksem71,$fucksem81,$explode_fuck,$kojoSen2,$program,$year,$programme,$dpt3,$fac3,$lectname, $preyear, $combine) 
                {
                        ...
                        $sheet->setCellValue(''.$statsClass.'8','Abandoned : ');
                        $sheet->setCellValue(''.$statsClass.'9','Withdrawn : ');
                        $sheet->setCellValue(''.$statsClass.'10','Deferred : ');
                        $sheet->setCellValue(''.$statsClass.'11','Rusticated : ');
                        $sheet->setCellValue(''.$statsClass.'12','Resit : ');
                        $sheet->setCellValue(''.$statsClass.'13','Unknown : ');
                        ...
                        $statsClassCount = ++$statsClass;
                        //$sheet->setCellValue(''.$statsClassCount.'3','No');
                        $sheet->setCellValue(''.$statsClassCount.'4','=COUNTIFS(RSA!AD8:RSA!AD'.$kwhykojoSense.',"CD")');
                        $sheet->setCellValue(''.$statsClassCount.'5','=COUNTIFS(RSA!AD8:RSA!AD'.$kwhykojoSense.',"CM")');
                        $sheet->setCellValue(''.$statsClassCount.'6','=COUNTIFS(RSA!AC8:RSA!AC'.$kwhykojoSense.',"C")');
                        $sheet->setCellValue(''.$statsClassCount.'7','=COUNTIFS(RSA!AC8:RSA!AC'.$kwhykojoSense.',"NC")');
                        $sheet->setCellValue(''.$statsClassCount.'8','=COUNTIFS(RSA!AD8:RSA!AD'.$kwhykojoSense.',"Abandoned")');
                        $sheet->setCellValue(''.$statsClassCount.'9','=COUNTIFS(RSA!AD8:RSA!AD'.$kwhykojoSense.',"Withdrawn")');
                        $sheet->setCellValue(''.$statsClassCount.'10','=COUNTIFS(RSA!AC8:RSA!AC'.$kwhykojoSense.',"Deferred")');
                        $sheet->setCellValue(''.$statsClassCount.'11','=COUNTIFS(RSA!AC8:RSA!AC'.$kwhykojoSense.',"Rusticated")');
                        $sheet->setCellValue(''.$statsClassCount.'12','='.$TotallAllstudent.'-(SUM('.$statsClassCount.'4:'.$statsClassCount.'11))');
                        $sheet->setCellValue(''.$statsClassCount.'13',0);
               ...
               //   # code...
                        ...
                        if ($preyear > 5) {
                            ...
                            $sheet->cells(''.$statsClassCount.'10:'.$statsClassCount.'20', function($cells) {
