...
class FeeController extends Controller
{
 ...
 //    }
    ...
    public function createBill(Request $request, SystemController $sys)
    {
        ...
        if ($sys->getUserLevel((@\Auth::user()->department),"create_bill") == '1' || $sys->getUserLevel((@\Auth::user()->role),"create_bill") == '1') {
    ...
    public function showPaid_excel(SystemController $sys){
        
            $faculty=$sys->getSchoolList();
            
            return view('students.paid_excel')->with('faculty', $faculty)
                ->with('level', $sys->getLevelList())->with('year',$sys->years());
        
        
    }

    public function paid_excel(Request $request, SystemController $sys )
...
foreach($programSelect as $row){
   ...
   $studentPay = Models\StudentModel::join('tpoly_programme', 'tpoly_students.PROGRAMMECODE', '=', 'tpoly_programme.PROGRAMMECODE')->where("tpoly_students.STATUS", 'In school')->where("tpoly_students.PROGRAMMECODE", $row->PROGRAMMECODE)->where("tpoly_students.LEVEL", $level_row->LEVEL)
    ...
    public function showOwing_excel(Request $request,SystemController $sys){
            ...
            return view('students.owing_excel')->with('faculty', $faculty)
                ->with('level', $sys->getLevelList())->with('year',$sys->years());
        
        
    }

    public function owing_excel(Request $request, SystemController $sys )

    {
       ...
       // dd($resultb);
        ...
        return Excel::create($partYear.' '.$faculty.' fee owing', function ($excel) use ($kojoSense, $sys, $yearcc, $faculty){
...
foreach($programSelect as $row){
   ...
   $studentPay = Models\StudentModel::join('tpoly_programme', 'tpoly_students.PROGRAMMECODE', '=', 'tpoly_programme.PROGRAMMECODE')->where("tpoly_students.STATUS", 'In school')->where("tpoly_students.PROGRAMMECODE", $row->PROGRAMMECODE)->where("tpoly_students.LEVEL", $level_row->LEVEL)->where("tpoly_students.BILL_OWING",">",0.00)
    ...
    public function masterLedger(Request $request, SystemController $sys)
    {
       ...
       // $sem = $array[0]->SEMESTER;
        ...
        if ($request->has('type') && trim($request->input('type'))) {
           // $fee->where("STUDENT->BILL_OWING", ">", 0);

            $fee->whereHas('student', function($q)use ($request) {
                if ($request->input('type') == 'Part payment') {
                 $q->where('BILL_OWING',">", 0);
            }
                 
               if ($request->input('type') == 'Full payment') {
                 $q->where('BILL_OWING',"<=", 0);
            }        
        ...
        $data = $fee->groupBy('STUDENT')->orderBy('PROGRAMME', 'ASC')->orderBy('LEVEL', 'ASC')->orderBy('INDEXNO', 'ASC')->paginate(1000);

        // if ($request->has('type') && trim($request->input('type'))) {
        //    if ($request->input('type') == 'Part payment') {
        //        dd("update in progress");
        //    }
        //    if ($request->input('type') == 'Full payment') {
        //        dd("update in progress");
        //    }
        //     //$fee->where("PAYMENTTYyPE", "=", $request->input('type'));
        // }
        // if ($request->has('type') && trim($request->input('type'))) {
    ...
    // approve bill here
    public function approvSe(Request $request, $id, SystemController $sys)
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"create_bill") == '1' || $sys->getUserLevel((@\Auth::user()->role),"create_bill") == '1') {
            $sys = new SystemController();
            $array = $sys->getSemYear();
...
//         }
    ...
    public function status(Request $request, SystemController $sys)
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"change_status") == '1' || $sys->getUserLevel((@\Auth::user()->role),"change_status") == '1') {
            if ($request->isMethod("get")) {
    ...
    public function processStatus(Request $request, SystemController $sys)
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"change_status") == '1' || $sys->getUserLevel((@\Auth::user()->role),"change_status") == '1') {
            ...
            $grad2 = '';
            $currentGrad = $sys->getStudentyeargroupfromIndexno($student);
                    $balance = @$balanc[0]->BALANCE;
                    $paid = @$balanc[0]->PAID;
                    $trail = @$balance[0]->TRAIL;//dd($level);
            ...
            if ($level == '100H' || $level == '100NT' || $level == '100BTT' || $level == '100BT') {
                $level_year = '1st';
            }
            if ($level == '200H' || $level == '200BT') {
                $level_year = '2nd';
            }
...
//dd($grad2);
            ...
            $gradSave = new Models\GradModel();
//dd($grad2);
            //$gradSave->year_group = $currentGrad;
            $gradSave->student = $student;
            $gradSave->action = $status;
            $gradSave->user = @\Auth::user()->fund;

            if ($status == 'In school') {
                Models\StudentModel::where("INDEXNO", $student)->update(array('STATUS' => $status,"ALLOW_REGISTER" => 1,"GRADUATING_GROUP" => $grad2,"LEVEL" => $level,"BILLS" => $billF,"YEAR" => $level_year,"BILL_OWING" => $owe));
                $gradSave->year_group = $grad2;
                $gradSave->save();
            }
            if ($status != 'In school') {
                Models\StudentModel::where("INDEXNO", $student)->update(array('STATUS' => $status,"LEVEL" => $level,"TRAIL" => 'yes',"ALLOW_REGISTER" => 0,"REGISTERED" => 0));
                $gradSave->year_group = $currentGrad;
                Models\GradModel::where("student", $student)->where("year_group", $currentGrad)->update(array('action' => $status));
            }
    ...
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"change_programme") == '1' || $sys->getUserLevel((@\Auth::user()->role),"change_programme") == '1') {
            if ($request->isMethod("get")) {
    ...
    public function printOldReceipt(Request $request, SystemController $sys)
    {

        if ($sys->getUserLevel((@\Auth::user()->department),"print_receipt") == '1' || $sys->getUserLevel((@\Auth::user()->role),"print_receipt") == '1') {
            if ($request->isMethod("get")) {
    ...
    public function printPasswordReceipt(Request $request, SystemController $sys)
    {
...
//echo $thea[0];
    ...
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"delete_fee") == '1' || $sys->getUserLevel((@\Auth::user()->role),"delete_fee") == '1') {
        \DB::beginTransaction();
        try {
    ...
    public function destroyPayment(Request $request, SystemController $sys)
    {
        if ($sys->getUserLevel((@\Auth::user()->department),"delete_payment") == '1' || $sys->getUserLevel((@\Auth::user()->role),"delete_payment") == '1') {
        \DB::beginTransaction();
        try {
