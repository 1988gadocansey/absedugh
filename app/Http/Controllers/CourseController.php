...
//dd($sys->getUserLevel((@\Auth::user()->department),"view_results_all"));
    ...
    public function generateTranscript($sql,  SystemController $sys){
        ...
        $records=  Models\AcademicRecordsModel::where('student','=',$sql)->where('grade','!=', 'E')->where('grade','!=', 'IC')->where('grade','!=', 'NC')->groupBy("year")->groupBy("level")->orderBy("level")->get();
            ...
            <tr>
                ...
                <td  style=" " align="left">
                    ...
                    foreach ($records as $row){
                        for($i=1;$i<3;$i++){
                            $query=  Models\AcademicRecordsModel::where("student",$sql)->where('total','>',0)->where("year",$row->year)->where("sem",$i)->orderby("code")->orderby("resit")->get()->toArray();
...
<table width="350px" style="margin-left:18px" height="133">
                                
                                
                                <tr>
                                    <td width = "250px" class="uk-text-bold"style="vertical-align: center;">Grade
                                    </td>
                                    <td width = "8px">&nbsp;
                                    </td> 
                                    <td class="uk-text-bold">Range
                                    </td>
                                    
                                </tr>
                                <tr>
                                    <td>CD&nbsp;&nbsp;-&nbsp;&nbsp;Competent with Distinction
                                    </td> 
                                    <td>-
                                    </td>
                                    <td>80&nbsp; : &nbsp;100
       ...
       // dd($resultb);
        ...
        return Excel::create($course.'_'.$programme, function ($excel) use ($data,$program,$level,$programme,$kojoSense,$course,$dpt3,$fac3,$courced,$lectname,$yearCurrent,$semCurrent){
            ...
            $excel->sheet($program, function ($sheet) use ($data,$kojoSense,$program,$level,$programme,$course,$dpt3,$fac3,$courced,$lectname,$yearCurrent,$semCurrent) {
                ...
                $sheet->setBorder('A1:I'.$kojoSense.'', 'thin');
                ...
                $sheet->getHeaderFooter()->setOddFooter('&L&B &RPage &P of &N');
                $sheet->getHeaderFooter()->setOddFooter('&L&B &LLecturer: '.$lectname.'&C&HSignature: _____________________'.'&L&B &RPage &P of &N');
                ...
                $sheet->setCellValue('F3','Semester :');
                $sheet->setCellValue('F4','Level :');
                $sheet->setCellValue('F5','Course Code :');

                $sheet->setCellValue('H2',$yearCurrent);
                $sheet->setCellValue('H3',$semCurrent);
                $sheet->setCellValue('H4',$level);
                $sheet->setCellValue('H5',$course);

                $statsBegin = $kojoSense + 8;
                $statsEnd = $kojoSense + 16;
                $kStats = $kojoSense + 8;

                //$sheet->setCellValue('J3','STATISTICS');
                $sheet->setCellValue('E'.$kStats,'Max :');
                $sheet->setCellValue('F'.$kStats,'=MAX(H8:H'.$cheat.')');
                $sheet->setCellValue('H'.$kStats,'A+ :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"A+")');

                $sheet->setCellValue('B'.$kStats,'Lecturer :');
                $sheet->setCellValue('C'.$kStats,$lectname);

                $sheet->cells('E'.$kStats.':F'.$kStats, function($cells) {

                    $cells->setBackground('#262626');
                    $cells->setFontColor('#edeff6');
                 });

                $sheet->cells('H'.$kStats.':I'.$kStats, function($cells) {

                    $cells->setBackground('#595959');
                    $cells->setFontColor('#edeff6');
                 });

                $kStats +=1;
                $sheet->setCellValue('E'.$kStats,'Min :');
                $sheet->setCellValue('F'.$kStats,'=MIN(H8:H'.$cheat.')');
                $sheet->setCellValue('H'.$kStats,'A :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"A")');
                ...
                $kStats +=1;
                $sheet->setCellValue('E'.$kStats,'Avg :');
                $sheet->setCellValue('F'.$kStats,'=IFERROR(AVERAGE(H8:H'.$cheat.'),"")');
                $sheet->setCellValue('H'.$kStats,'B+ :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"B+")');

                $sheet->setCellValue('B'.$kStats,'Date :');
                $sheet->setCellValue('C'.$kStats,'_____________________');
                ...
                $kStats +=1;
                $sheet->setCellValue('H'.$kStats,'C+ :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"C+")');
                
                $sheet->setCellValue('B'.$kStats,'Lecturer Sign :');
                $sheet->setCellValue('C'.$kStats,'_____________________');
                ...
                $kStats +=1;
                $sheet->setCellValue('H'.$kStats,'D+ :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"D+")');
                
                $sheet->setCellValue('B'.$kStats,'HOD Name :');
                $sheet->setCellValue('C'.$kStats,'_____________________');
                $sheet->setCellValue('D'.$kStats,'HOD Date : __________________');
                ...
                $kStats +=1;
                $sheet->setCellValue('H'.$kStats,'F :');
                $sheet->setCellValue('I'.$kStats,'=COUNTIF(I8:I'.$cheat.',"F")');
                
                $sheet->setCellValue('B'.$kStats,'HOD Sign :');
                $sheet->setCellValue('C'.$kStats,'_____________________');
                ...
                $kStats +=1;
                $sheet->setCellValue('H'.$kStats,'Sum :');
                $sheet->setCellValue('I'.$kStats,'=SUM(I'.$statsBegin.':I'.$statsEnd.')');
               ...
               // $sheet->setCellValue('G'.$k.'','=SUM(C'.$k.':F'.$k.')');
                ...
                for($lisa=1;$lisa<6;$lisa++){
                    ...
                    $sheet->mergeCells('H'.$lisa.':I'.$lisa);
                    ...
                    $sheet->cells('A1:I6', function($cells) {
                ...
                $sheet->cells('F2:F5', function($cells) {

                   $cells->setAlignment('right');
                });

                $sheet->cells('I8:I'.$cheat.'', function($cells) {
                ...
                $sheet->cells('F'.$statsBegin.':F'.$kStats, function($cells) {

                   $cells->setAlignment('left');
                });

                $sheet->cells('B'.$statsBegin.':B'.$kStats, function($cells) {

                   $cells->setAlignment('right');
                });

                $sheet->cells('I'.$statsBegin.':I'.$kStats, function($cells) {
                                        ...
                                        if (!empty($data) && $data->count()) 
                {
                        ...
                        foreach ($data as $key => $value) 
                    {
                        ...
                        if(substr($indexNo,0,1)==7){
                            ...
                            $totalRecords = count($data) - 11;
                            $essien = $essien + 1;
                            //dd($totalRecords);
                            if ($essien <= $totalRecords) {
                           //     dd($indexNo);
                           // }
...
//                        if (@in_array($studentDb, $studentSearch)) {
                            ...
                            $midsem=@$value->midsem;
                            $exam=@$value->exam;
                            $total= @round(($quiz1+$quiz2+$midsem+$exam),2);
