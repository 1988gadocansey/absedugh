...

{
...

    {
      if ($sys->getUserLevel((@\Auth::user()->department),"academic_calender") == '1' || $sys->getUserLevel((@\Auth::user()->role),"academic_calender") == '1') {
        ...
        $calender = AcademicCalenderModel::select([  'ID','YEAR', 'SEMESTER','STATUS','ENTER_RESULT','LIAISON','QA','RESULT_BLOCK','QAOPEN','RESULT_DATE','GRAD','ADMIT'])->orderBy('YEAR','DESC')->paginate('100');
    ...
    public function storeCalender(Request $request, SystemController $sys)

    {
      ...
      $admit=$request->input('admit');
          ...
          $calender= AcademicCalenderModel::where("id",9)->update(array('YEAR'=>$year, 'SEMESTER'=>$sem, 'RESULT_DATE'=>$upload, 'QA'=>$qa, 'RESULT_BLOCK'=>$result, 'GRAD'=>$grad, 'ADMIT'=>$admit));
